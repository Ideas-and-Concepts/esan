...
def record_payment():
    ...
    try:
        ...
        with st.form("payment_form"):
            selected = st.selectbox("Invoice", list(invoice_options.keys()))
            amount = st.number_input("Payment Amount", min_value=0.0, step=0.01)
            method = st.selectbox("Method", ["Cash", "Bank Transfer", "Mobile Money", "Cheque", "Credit Card", "Other"])
            ...
            if st.form_submit_button("Record Payment"):
                if amount <= 0:
                    ...
                    return
                new_payment = Payment(
                    invoice_id=invoice_options[selected],
                    amount=amount,
                ...
                db.add(new_payment)
                inv = db.query(Invoice).filter(Invoice.id == invoice_options[selected]).first()
                if inv:
...
def payment_summary():
    ...
    try:
        ...
        if payments:
            ...
            st.bar_chart(pd.DataFrame(list(method_totals.items()), columns=['Method', 'Amount']).set_index('Method'))
        else:
