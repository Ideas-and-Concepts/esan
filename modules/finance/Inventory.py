...
def create_invoice():
    ...
    try:
        ...
        with st.form("invoice_form"):
            ...
            order = db.query(SalesOrder).filter(SalesOrder.id == order_options[selected_order]).first()
            if order:
    ...
    try:
        ...
        if invoices:
            ...
            if selected_invoice_num:
                inv = db.query(Invoice).filter(Invoice.id == invoice_options[selected_invoice_num]).first()
                if inv:
                    customer = db.query(Customer).filter(Customer.id == inv.customer_id).first()
